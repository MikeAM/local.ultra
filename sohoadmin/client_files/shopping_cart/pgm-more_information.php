...
include_once('../sohoadmin/client_files/qty_disc-more_info.php');
...
if ( (!file_exists($TEST_IMAGE) || $THIS_IMAGE == "../images/") && !eregi('http://', $THIS_IMAGE) ) {
...
$THIS_DISPLAY .= "      <br/><div align=\"center\" class=\"smtext\">".lang("Add this product to your cart below.")."</div>\n";
...
eval(hook("pgm-more_information.php:addninfocontent"));
      ...
      if($qtyarray['required_qty'] > 1){
        $quantityval = $qtyarray['required_qty'];
      } else {
        $quantityval = '1';
      }
   ...
   /*---------------------------------------------------------------------------------------------------------*/
	if ( strlen($PROD['other_images']) > 2 || strlen($PROD['PROD_FULLIMAGENAME']) > 2 || $PROD[OPTION_DETAILPAGE] != "") {	// If more info exists
      ...
      if ( strlen($PROD['PROD_FULLIMAGENAME']) > 2 ) {
         ...
         if ( !eregi('http://', $PROD['PROD_FULLIMAGENAME']) ) {
         	$fullsize_url = "../images/".$PROD['PROD_FULLIMAGENAME'];
         } else {
         	$fullsize_url = $PROD['PROD_FULLIMAGENAME'];
         }
...
eval(hook("pgm-more_information.php:after-search-column"));

#################################################################################
### BUILD OVERALL TABLE TO PLACE SEARCH COLUMN TO THE LEFT OR RIGHT OF
### SEARCH RESULT DISPLAY AS DEFINED IN DISPLAY OPTIONS
#################################################################################
if($PROD['PROD_CATEGORY1'] != ''){
	if($catz[$PROD['PROD_CATEGORY1']]['name'] != ''){
		$cat = $PROD['PROD_CATEGORY1'];
