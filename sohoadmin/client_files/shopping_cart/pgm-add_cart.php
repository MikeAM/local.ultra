...
//			   $reqfield_js .= "            alert('check: ['+rfld[f]+']('+rfld[f].charCodeAt(14)+')');\n";
			   ...
			   $reqfield_js .= "               err_msg += 'Item '+y+': '+rfld[f].replace(/_/g, \" \")+'\\n';\n";
			   $reqfield_js .= "            }\n";
...
eval(hook("pgm-add_cart.php:fetchtmparray"));
   ...
   include_once('../sohoadmin/client_files/qty_disc-add_cart.php');
   ...
   $THIS_DISPLAY .= "  </th>\n";
	...
	include_once('../sohoadmin/client_files/cart_discount.php');
   ...
   if ($line_items != -1) {
      ...
      for ($z=0;$z<=$line_items;$z++) {
		...
		$display_discount = $display_discount + sprintf ("%01.2f", $_SESSION['unitsub_array'][$z]);
...
eval(hook("pgm-add_cart.php:jsvars"));
	...
	$display_subtotal = $display_subtotal - $display_discount;
