...
$uninstalled_plugin_name=$_GET['uninstalled_plugin_name'];
...
if(!$uninstalled_plugin_name) {
	$sterilized_filename = eregi_replace('[\(\)]', '', $_FILES['FILE1']['name']);
   ...
   if ( isset($_GET['downloaded_zipfile']) ) {
      ...
      $uploadFile = $plugins_dir_path . "temp/" . $sterilized_filename;
      $zipfile_name = $sterilized_filename;
      $OS = strtoupper(PHP_OS);
   }
...
//   echo '['.testArray($_FILES['FILE1']).']<br/>'; exit;
...
if($uninstalled_plugin_name) {
	$plugin_folder_name = $uninstalled_plugin_name;
}
...
if ( $plugin_install_form != "" && $_GET['todo'] != "finish_install" ) {
	header("location:../../../plugins/".basename($plugin_folder_name)."/".$plugin_install_form); exit;
...
if ( !addon_licensed($plugin_folder_name, 'install') ) {
