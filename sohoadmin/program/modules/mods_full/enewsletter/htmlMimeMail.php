...
*/
	...
	function htmlMimeMail()
	{
        ...
        */
		...
		$smtp_hostname 	=	$newspref->get("smtp_hostname");
		$smtp_username 	=	$newspref->get("smtp_username");
		$smtp_password 	=	$newspref->get("smtp_password");
		$smtp_port 		=	$newspref->get("smtp_port");

		if($newspref->get("smtp_custom") == "yes"){
			if($smtp_hostname != ''){
				$helo = $smtp_hostname;
			} elseif (!empty($GLOBALS['HTTP_SERVER_VARS']['HTTP_HOST'])) {
			...
			$this->smtp_params['host'] = $smtp_hostname;
			$this->smtp_params['port'] = $smtp_port;
			$this->smtp_params['helo'] = $helo;
			$this->smtp_params['auth'] = true;
			$this->smtp_params['user'] = $smtp_username;
			$this->smtp_params['pass'] = $smtp_password;
		...
		switch ($type) {
            ...
            //error_reporting(E_ALL);
				if (!empty($this->return_path)) {
					...
					//$result = mail($to, $subject, "This is my super awesome output!.\nIt has a line break!");
				} else {
				...
				echo "output:<br> <textarea style=\"width: 450px; height: 300px;\">".$this->output."<br><br>";
				echo "output:<br> <textarea style=\"width: 450px; height: 300px;\">".implode(CRLF, $headers)."<br>";
				exit;
			...
			case 'smtp':
				...
				if (!$smtp->send($send_params)) {
					...
					echo "<br><b>Error Details</b>...<br>";
					foreach ( $this->errors as $var=>$val ) {
					   echo $val."<br>\n";
					}
					echo "The from address you provided may be incorrect";
					exit;
					return false;
				}
				return true;
